...
const HrValueForm = ({ hrValueCreate, setOpenPopup, hrValueEdit }) => {
  ...
  const validate = (fieldValues = values) => {
    ...
    temp.SchoolCode = !fieldValues.SchoolCode ? "This Field is Required" : "";
          ...
          <Grid container style={{ fontSize: "12px" }}>
            <Grid item xs={6}>
              ...
              <InputControl
                name="SchoolCode"
                label="School Code*"
                disabled={values.IDHRCompanyValue === 0 ? false : true}
                value={values.SchoolCode}
                onFocus={(e) => {
                  e.target.select();
                }}
                onChange={handleInputChange}
                errors={errors.SchoolCode}
              />
              <InputControl
                name="FullAddress"
                label="Full Address*"
              ...
              <img
                src={
                  imgSrc1
                    ? imgSrc1
                    : hrValueEdit &&
                      `${API_URL}${hrValueEdit.FullPathSchoolLogo}`
                }
                height={200}
                width={200}
              />
